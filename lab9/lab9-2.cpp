...
void distribute_data(int m, int n, int num_procs, int proc_rank,
...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        for (int i = 0; i < m * n; ++i)
        {
            A[i] = static_cast<double>(rand() % 100) / RAND_MAX;
            B[i] = static_cast<double>(rand() % 100) / RAND_MAX;
        }
    }
    ...
    distribute_data(m, n, size, rank, A, B, local_A, local_B);

    for (int i = 0; i < rank; ++i)
    { // такое заполнение равномерное в отличии от "свалить все на последнего"
